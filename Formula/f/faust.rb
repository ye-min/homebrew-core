class Faust < Formula
  ...
  patch do
    url "https://github.com/grame-cncm/faust/commit/5b275806093fb2e3348043de1150f01ff892d232.patch?full_index=1"
    sha256 "f5458dae770b6b96da7ebe22e8b928be6f2e4932b5af6fe7af9fa1121edd1ab6"
  end
  ...
  def install
    ...
    # Avoid linkage to `z3` when using latest `llvm`
    args += %w[EXE SHARED].map { |type| "-DCMAKE_#{type}_LINKER_FLAGS=-Wl,-dead_strip_dylibs" } if OS.mac?

    system "cmake", "-S", "build", "-B", "homebrew_build", *args, *std_cmake_args
